<x-admin-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Modificar Temporada: <span class="text-indigo-600 font-bold">{{ $season->name }}</span>
        </h2>
    </x-slot>
                ...
                <form action="{{ route('admin.seasons.update', $season) }}" method="POST">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $season->name)" required />
                        ...
                        <div>
                            ...
                            <x-select id="type" name="type" class="mt-1 block w-full" required>
                                <option value="mid" @selected(old('type', $season->type) == 'mid')>Temporada Media</option>
                                <option value="high" @selected(old('type', $season->type) == 'high')>Temporada Alta</option>
                            </x-select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-input id="start_date" name="start_date" type="date" class="mt-1 block w-full" :value="old('start_date', $season->start_date?->toDateString())" required />
                                <x-input-error for="start_date" class="mt-2" />
                            </div>

                            <div>
                                <x-label for="end_date" value="Fecha de Fin" class="font-medium text-gray-700" />
                                <x-input id="end_date" name="end_date" type="date" class="mt-1 block w-full" :value="old('end_date', $season->end_date?->toDateString())" required />
